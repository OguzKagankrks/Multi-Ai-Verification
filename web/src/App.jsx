...
const LOCALE = {
  tr: {
    waiting: 'Bekleniyor...',
    skip: 'API anahtarı tanımlı değil, model atlandı.',
    finalMissing: 'ChatGPT anahtarı olmadığı için nihai cevap oluşturulamadı.',
    errorPrefix: 'Hata',
    emptyAnswer: 'Boş yanıt',
    emptyCard: name => 'Bu alanda ' + name + ' sonucunu göreceksin.',
    steps: {
      gemini: 'Gemini Yanıtı',
      claude: 'Claude Değerlendirmesi',
      grok: 'Grok Doğrulaması',
      perplexity: 'Perplexity Analizi',
      chatgpt: 'ChatGPT İncelemesi',
      final: 'Son Cevap'
    },
    header: {
      themeLabel: { light: 'Gündüz modu', dark: 'Gece modu' },
      languageLabel: 'Dil',
      languages: [
    ...
    input: {
      ...
      hint: 'VS Code: Terminalde "npm run dev" komutunu çalıştır. İlk seferde Ayarları açıp anahtarları kontrol et.',
      send: 'Gönder'
    },
    sidePanel: {
      title: 'Hızlı Sohbet',
      optionsLabel: 'Seçenekler',
      placeholder: 'Sağ panelde seçili modele sorunuzu yazın...',
      send: 'Gönder',
      selectModel: 'Lütfen önce bir model seç.',
      emptyResponse: 'Henüz yanıt yok.',
      responseTitle: 'Son Yanıt',
      missingLabel: 'API anahtarı yok',
      modelNames: {
        gemini: 'Gemini',
        claude: 'Claude',
        grok: 'Grok',
    ...
    about: {
      title: 'Hakkında',
      description: 'Multi-Ai Flow, bir soruya birden fazla modeli sıra ile çalıştırarak doğrulama ve nihai yanıt üretimini otomatikleştirir.',
      featuresTitle: 'Özellikler',
      features: [
        'Gemini sabit sıranın ilk adımında yanıt oluşturur',
        'Claude, Grok ve Perplexity ardıl adımlarda yanıtı denetler',
        'ChatGPT yoksa final kartı son başarılı modelin sonucunu gösterir',
        'API hatası alan adım kartta hata mesajı gösterir ama zincir devam eder',
        'Sağ panelden istediğin modeli tek başına çalıştırabilirsin'
      ],
      usageTitle: 'Nasıl kullanılır?',
      usageSteps: [
        'Sol sütundaki formdan sorunu yazıp Gönder düğmesine bas.',
        'Kartlar sırayla dolarken her adımdaki uyarıları kontrol et.',
        'ChatGPT anahtarı yoksa final kartında en son çalışan modelin yanıtını görürsün.',
        'Sağ paneli kullanarak aynı soruyu farklı modellerle doğrudan test et.'
      ]
    },
    status: { ready: 'hazır', disabled: 'pasif' },
    copy: { button: 'Kopyala', success: 'Kopyalandı' },
    settings: {
      title: 'API Anahtarları',
      statusIntro: 'Sunucuda kayıtlı anahtar durumu:',
      statusOrder: [
      ...
      fields: {
        ...
        xaiTeamId: { label: 'xAI Team ID', placeholder: 'Opsiyonel ekip kimliği', type: 'text' },
      ...
      buttons: { clear: 'Temizle', close: 'Kapat', save: 'Kaydet' }
    },
    placeholders: {
      gemini: '[Gemini atlandı]',
      claude: '[Claude atlandı]',
      grok: '[Grok atlandı]',
      perplexity: '[Perplexity atlandı]'
    },
    prompts: {
  ...
  en: {
    waiting: 'Waiting...',
    skip: 'API key missing; step skipped.',
    finalMissing: 'Final answer unavailable because the ChatGPT key is missing.',
    errorPrefix: 'Error',
    emptyAnswer: 'Empty response',
    emptyCard: name => 'This area will show the ' + name + '.',
    steps: {
      gemini: 'Gemini Response',
      claude: 'Claude Review',
      grok: 'Grok Verification',
      perplexity: 'Perplexity Analysis',
      chatgpt: 'ChatGPT Review',
      final: 'Final Answer'
    },
    header: {
      themeLabel: { light: 'Day mode', dark: 'Night mode' },
    ...
    input: {
      placeholder: 'Type your question... (Enter: Send)',
      hint: 'VS Code: run "npm run dev" in the terminal. Open Settings to verify your API keys the first time.',
      send: 'Send'
    },
    sidePanel: {
      title: 'Quick Chat',
      optionsLabel: 'Options',
      placeholder: 'Type a message for the selected model...',
      send: 'Send',
      selectModel: 'Please choose a model first.',
      emptyResponse: 'No reply yet.',
      responseTitle: 'Latest Reply',
      missingLabel: 'API key missing',
      modelNames: {
        gemini: 'Gemini',
        claude: 'Claude',
        grok: 'Grok',
    ...
    about: {
      title: 'About',
      description: 'Multi-Ai Flow runs a fixed pipeline of models to vet an answer and expose the most reliable result.',
    ...
    settings: {
      ...
      fields: {
        gemini: { label: 'Gemini', placeholder: 'Enter your key...', type: 'password' },
        claude: { label: 'Claude', placeholder: 'Enter your key...', type: 'password' },
        xai: { label: 'Grok (xAI)', placeholder: 'Enter your key...', type: 'password' },
        xaiTeamId: { label: 'xAI Team ID', placeholder: 'Optional team identifier', type: 'text' },
        perplexity: { label: 'Perplexity', placeholder: 'Enter your key...', type: 'password' },
        openai: { label: 'OpenAI (ChatGPT)', placeholder: 'Enter your key...', type: 'password' }
      },
...
export default function App() {
  ...
  const copyStrings = locale.copy ?? { button: 'Copy', success: 'Copied' }
  ...
  const handleCopy = useCallback(async (cardId, rawValue) => {
    ...
    try {
      ...
      setCopiedState(prev => ({ ...prev, [cardId]: true }))
      if (copyTimeouts.current[cardId]) {
        window.clearTimeout(copyTimeouts.current[cardId])
      }
      copyTimeouts.current[cardId] = window.setTimeout(() => {
        setCopiedState(prev => {
          const next = { ...prev }
          delete next[cardId]
          return next
        })
        delete copyTimeouts.current[cardId]
      }, 2000)
    } catch (_error) {
  ...
  const mainComposerTitle = language === 'tr' ? 'Ana Akış Sorusu' : 'Pipeline Question'
  const sendingLabel = language === 'tr' ? 'Gönderiliyor...' : 'Sending...'
  ...
  const handleMainSubmit = async event => {
    ...
    try {
      ...
      // errors surfaced through UI notifications
    } finally {
        ...
        <div className="main-column">
          ...
          <section className="stage" aria-live="polite">
            {orderedSteps.map(step => {
              ...
              const cardTextRaw = answers[step.id] ?? locale.emptyCard(step.name)
              const cardText = typeof cardTextRaw === 'string' ? cardTextRaw : String(cardTextRaw ?? '')
              const canCopy = cardText.trim().length > 0
              const isCopied = !!copiedState[step.id]
              const copyLabel = isCopied ? copyStrings.success : copyStrings.button
              const copyDisabled = !canCopy || (!stepAvailable && step.id !== 'final')
              ...
              return (
                <ModelCard
                  ...
                  text={cardText}
                  onCopy={() => handleCopy(step.id, cardText)}
                  copyLabel={copyLabel}
                  copyActive={isCopied}
